...
<body>
	...
	<%-- <c:set var="book" value="${sessionScope.shopping }"></c:set> --%>
	<div id="content" class="wrap">
		<div class="list bookList">
			<form method="post" name="shoping" action="../ordersServlet?oper=items">
				<table>
					<tr class="title">
					...
					<c:forEach var="item" items="${sessionScope.shopping.items}">
						<tr>
							<td class="thumb"><img src="${item.item.image}" /></td>
							<td class="title">${item.item.bookname}</td>
							<td><input class="input-text" type="text" name="nums" value="${item.count}" /></td>
							<td>￥${item.b_price}</td>
						</tr>
					</c:forEach>   
				</table>
				<div class="button">
					<h4>
						总价：￥${sessionScope.shopping.sumPrice}元
					</h4>
